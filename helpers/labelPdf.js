...
const labelPdf = (data) => {
  ...
  if (data.jumlahBarang <= 1) {
    ...
    doc.autoTable({
      ...
      body: [[{ content: 1 + " of " + data.jumlahBarang, styles: { fontStyle: "bold" } }]],
    });
    ...
    //kotak layanan | berat | Koli | pembayaran
    doc.autoTable({
      theme: "grid",
      ...
      body: [
        [
          ...
          { content: Number(data.jumlahBarang).toLocaleString("id-ID") + " Koli", styles: { fontStyle: "bold" } },
          { content: Number(data.beratBarang).toLocaleString("id-ID") + " Kg", styles: { fontStyle: "bold" } },
    ...
    doc.autoTable({
      ...
      body: [[{ content: "Tgl Transaksi: " + getDate(data.tglTransaksi) }]],
    });
  } else {
    for (let i = 1; i <= data.jumlahBarang; i++) {
      ...
      doc.addImage(imglogo, "PNG", 2, 2, 38.4, 7.36);
      doc.addImage(imgbarcode, "PNG", 5, 11, 70, 20);
